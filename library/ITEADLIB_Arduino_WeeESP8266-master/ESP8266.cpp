...
String ESP8266::getUrlData(String path, String host, uint8_t length)
{
    ...
    m_puart->println(length);
	
    data = recvString("\r\n\r\nOK", 5000);
    if (data.indexOf("\r\n\r\nOK") != -1) {
		data = "";
        m_puart->println(path);
		m_puart->println("Host: " + host);
		m_puart->println("Connection: close");
		m_puart->println("");
		m_puart->println("");
		
		data = recvString("CLOSED", 5000);
...
int ESP8266::getCommand(String path, String host, uint8_t length)
{
    /*
    0 = 讀取資料失敗 or 無命令 (NO|safeCommand or else)
    1 = 上 (UP|safeCommand)
    2 = 下 (DOWN|safeCommand)
    3 = 停 (STOP|safeCommand)
    4 = 斷電 (INT|safeCommand)
    */

    String data;
    data = getUrlData(path, host, length);

    if (data.indexOf("safeCommand") != -1) {
        if (data.indexOf("NO|") != -1) return 0;
        if (data.indexOf("UP|") != -1) return 1;
        if (data.indexOf("DOWN|") != -1) return 2;
        if (data.indexOf("STOP|") != -1) return 3;
        if (data.indexOf("INT|") != -1) return 4;
        return 0;
    } else {
        return 0;
    }
